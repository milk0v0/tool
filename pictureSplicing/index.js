...
PictureSplicing.prototype.onerror = function (img) {
  ...
  if (img.errorNum >= 3) {
    ...
    this.datas[img.index].error = true;
...
PictureSplicing.prototype.arrangement = function () {
  for (var i = this.datas.length - 1; i >= 0; i--) {
    (this.datas[i].error || this.datas[i].h >= this.opts.maxHeight) && this.datas.splice(i, 1);
  }
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    var y = 0;
    var x = (arr.length - i - 1) * this.opts.width;
